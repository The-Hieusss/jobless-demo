  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (form.profile_pic) {
      const { error } = await supabase.storage
        .from("profile-pics")
        .upload(`${user.id}/recruiter-profile.png`, form.profile_pic, { upsert: true });
      if (!error) {
        const { data: url } = supabase.storage.from("profile-pics").getPublicUrl(`${user.id}/recruiter-profile.png`);
        profile_pic_url = url.publicUrl;
      }
    }

    // Upload resume (optional for recruiters)
    let resume_url = null;
    if (form.resume) {
      const { error } = await supabase.storage
        .from("resumes")
        .upload(`${user.id}/recruiter-resume.pdf`, form.resume, { upsert: true });
      if (!error) {
        const { data: url } = supabase.storage.from("resumes").getPublicUrl(`${user.id}/recruiter-resume.pdf`);
        resume_url = url.publicUrl;
      }
    }

    // Save profile + recruiter details
    ...
    await supabase.from("recruiter_details").upsert({
      ...
      looking_for: form.looking_for ? form.looking_for.split(",").map(i => i.trim()) : [],
      preferred_majors: form.preferred_majors ? form.preferred_majors.split(",").map(i => i.trim()) : [],
      work_type: form.work_type,
    });

    // ✅ Save habits into recruiter_habits
    await supabase.from("recruiter_habits").upsert({
      id: user.id,
    ...
    // Show "Let's Match" screen for 3s, then go to swipe
    setTimeout(() => {
  ...
  const groups = [
    {
      ...
      choices: ["Skills and Competence", "Adaptive and Flexibility", "Creativity and Growth"],
    },
    {
      label: "What do you look for in a candidate’s work style?",
      key: "workStyle" as const,
      choices: [
        "Fast Paced and Agile", "Risk Taking", "Active and Engage",
        "Detail Oriented", "Calm Under Pressure", "Collaborative",
      ],
    },
    {
    ...
    },
    {
      ...
      choices: [
        "Confidence and Initiatives", "Strategic Thinking", "Risk Taking Mindset",
        "Strong Communication", "Decisive and Fast Thinker", "Ability to Inspire",
      ],
    },
  ];
    ...
    <div className="relative flex min-h-screen bg-gradient-to-b from-jobless-white to-jobless-blue/40 items-center justify-center px-4">
      ...
      {step === 1 && (
        ...
        >
          ...
          <input name="company_name" placeholder="Company Name" onChange={updateField}
            className="w-full rounded-full p-3 bg-white font-body"/>
          <input name="company_industry" placeholder="Company Industry" onChange={updateField}
            className="w-full rounded-full p-3 bg-white font-body"/>
          <input name="user_role" placeholder="Your Role" onChange={updateField}
            className="w-full rounded-full p-3 bg-white font-body"/>
          <input name="looking_for" placeholder="Looking For (comma separated)" onChange={updateField}
            className="w-full rounded-full p-3 bg-white font-body"/>
          <input name="preferred_majors" placeholder="Preferred Majors (comma separated)" onChange={updateField}
            className="w-full rounded-full p-3 bg-white font-body"/>
          <input name="work_type" placeholder="Work Type (remote, hybrid, in-person)" onChange={updateField}
            className="w-full rounded-full p-3 bg-white font-body"/>
          <textarea name="bio" placeholder="Short Bio" onChange={updateField}
          ...
          <label className="flex items-center gap-3 w-full rounded-full px-4 py-3 bg-white cursor-pointer">
            <input
              ...
              accept="image/*"
              className="hidden"
              onChange={(e) => setForm({ ...form, profile_pic: e.target.files?.[0] || null })}
            />
        ...
        >
          ...
          {groups.map((group) => (
            <div key={group.key} className="space-y-2">
              <h2 className="font-body font-semibold text-jobless-blue">{group.label}</h2>
